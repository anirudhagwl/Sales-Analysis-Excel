...
# =====================================================================
# SHEET 1: RAW DATA — All rows with formula-based computed columns
# =====================================================================
ws = wb.active
ws.title = 'Raw Data'
...
for i, (_, row) in enumerate(df.iterrows(), 2):
    ...
    # FORMULA: Delivery Days = Ship Date - Order Date
...
style_data_rows(ws, 2, LAST, len(raw_cols))
ws.freeze_panes = 'A2'
set_col_widths(ws, [8,16,12,12,14,14,22,12,14,16,16,10,10,14,16,40,12,10,10,12,12,12])
print(f'Raw Data: {N} rows written with formula columns')

# =====================================================================
# SHEET 2: SUMMARY STATISTICS — All formulas referencing Raw Data
# =====================================================================
ws2 = wb.create_sheet('Summary Statistics')
add_title(ws2, 'Descriptive Statistics — Computed with Excel Formulas', 1, 1)

# References to Raw Data columns
# Q = Sales (col 17), T = Profit (col 20), R = Quantity (col 18), S = Discount (col 19), U = Delivery Days (col 21)
...
stats = [
    ...
    ('Min', 'MIN'),
    ('Max', 'MAX'),
    ('Q1 (25th Pctl)', 'PERCENTILE'),
    ('Q3 (75th Pctl)', 'PERCENTILE'),
    ('Variance', 'VAR'),
]

for r, (label, func) in enumerate(stats, 4):
    ws2.cell(row=r, column=1, value=label)
    for c, (col_name, col_letter) in enumerate(ref_map.items(), 2):
        rng = f"'Raw Data'!{col_letter}2:{col_letter}{LAST}"
        if func == 'PERCENTILE' and label == 'Q1 (25th Pctl)':
            formula = f'=PERCENTILE({rng},0.25)'
        elif func == 'PERCENTILE' and label == 'Q3 (75th Pctl)':
        ...
        if col_name in ('Sales', 'Profit'):
            ...
            ws2.cell(row=r, column=c).number_format = PCT_FMT

# IQR row (formula: Q3 - Q1)
r_iqr = 14
...
for c in range(2, len(stat_headers) + 1):
    ...
    if c <= 3:
        ws2.cell(row=r_iqr, column=c).number_format = CURRENCY_FMT

style_data_rows(ws2, 4, r_iqr, len(stat_headers))
set_col_widths(ws2, [18, 16, 16, 14, 14, 14])

# --- Segment breakdown with formulas ---
seg_row = 17
add_title(ws2, 'Performance by Customer Segment', seg_row, 1)
seg_h = ['Segment', 'Total Sales', 'Total Profit', 'Profit Margin', 'Order Count']
for c, h in enumerate(seg_h, 1):
    ws2.cell(row=seg_row + 2, column=c, value=h)
style_header(ws2, seg_row + 2, len(seg_h))

segments = ['Consumer', 'Corporate', 'Home Office']
for r, seg in enumerate(segments, seg_row + 3):
...
for c in range(2, 6):
    cl = get_column_letter(c)
    if c == 4:
        ws2.cell(row=tr, column=c, value=f'=IF(B{tr}=0,0,C{tr}/B{tr})')
        ws2.cell(row=tr, column=c).number_format = PCT_FMT
    else:
        ws2.cell(row=tr, column=c, value=f'=SUM({cl}{seg_row+3}:{cl}{seg_row+5})')
        if c <= 3:
            ws2.cell(row=tr, column=c).number_format = CURRENCY_FMT
    ws2.cell(row=tr, column=c).font = Font(name='Arial', bold=True, size=10)

style_data_rows(ws2, seg_row + 3, tr, len(seg_h))

# =====================================================================
# SHEET 3: REVENUE ANALYSIS — yearly/monthly with formulas
# =====================================================================
ws3 = wb.create_sheet('Revenue Analysis')
add_title(ws3, 'Revenue Trend Analysis', 1, 1)

years = sorted(df['Year'].unique())
rev_h = ['Year', 'Total Revenue', 'Total Profit', 'Total Orders', 'Avg Order Value', 'Profit Margin', 'YoY Revenue Growth']
for c, h in enumerate(rev_h, 1):
    ws3.cell(row=3, column=c, value=h)
style_header(ws3, 3, len(rev_h))

# Write year labels, then use SUMIFS/COUNTIFS formulas
# Raw Data col C = Order Date (text YYYY-MM-DD), we need to match year
# Since dates are stored as strings, we'll use a helper approach: write year values
# and use SUMPRODUCT with YEAR() — but dates are text. Let's write numeric dates instead.
# Actually the dates in Raw Data are text strings. Let me use SUMPRODUCT with LEFT().

for r, year in enumerate(years, 4):
    ws3.cell(row=r, column=1, value=year)
    # Use SUMPRODUCT to match year from text date column C
...
for c in [2, 3, 4]:
    cl = get_column_letter(c)
    ws3.cell(row=tr, column=c, value=f'=SUM({cl}4:{cl}{yr_end_row})')
...
set_col_widths(ws3, [10, 16, 16, 14, 16, 14, 18])

# Bar chart — placed well below the table
chart = BarChart()
chart.type = "col"
...
chart.width = 20
chart.height = 13
cats = Reference(ws3, min_col=1, min_row=4, max_row=yr_end_row)
chart.add_data(Reference(ws3, min_col=2, min_row=3, max_row=yr_end_row), titles_from_data=True)
chart.add_data(Reference(ws3, min_col=3, min_row=3, max_row=yr_end_row), titles_from_data=True)
chart.set_categories(cats)
ws3.add_chart(chart, f"A{tr + 3}")

# Monthly breakdown — starts well after the chart
mon_start = tr + 22
...
# Pre-compute monthly data from pandas (these are source values for formulas above them)
...
for yr_idx, year in enumerate(years):
    ...
    ws3.cell(row=r, column=14).font = Font(name='Arial', bold=True, size=10)
...
ws3.add_chart(line_chart, f"A{mon_end + 3}")
...
# By Category — with SUMIF formulas
...
tr = 9
ws4.cell(row=tr, column=1, value='TOTAL').font = Font(name='Arial', bold=True, size=10)
for c in [2, 3, 5]:
    cl = get_column_letter(c)
    ws4.cell(row=tr, column=c, value=f'=SUM({cl}6:{cl}8)')
    ws4.cell(row=tr, column=c).font = Font(name='Arial', bold=True, size=10)
...
# Pie chart for category sales — to the right, no overlap
pie = PieChart()
...
ws4.add_chart(pie, "G3")

# By Sub-Category — with formulas
sub_start = 12
...
# Sub-category bar chart — placed below sub-category table
bar = BarChart()
bar.type = "col"
...
ws4.add_chart(bar, f"A{sub_end + 3}")

# =====================================================================
# SHEET 5: CUSTOMER SEGMENTATION (RFM)
# =====================================================================
ws5 = wb.create_sheet('Customer Segmentation')
add_title(ws5, 'RFM Customer Segmentation Analysis', 1, 1)
ws5.cell(row=2, column=1, value='Recency = Days since last purchase | Frequency = Number of orders | Monetary = Total spend').font = Font(name='Arial', size=9, italic=True, color='666666')
...
rfm['Segment'] = rfm['RFM_Score'].apply(lambda s: 'Champions' if s>=10 else 'Loyal Customers' if s>=8 else 'Potential Loyalists' if s>=6 else 'At Risk' if s>=4 else 'Lost')
...
rfm_h = ['Customer ID', 'Customer Name', 'Recency (days)', 'Frequency', 'Monetary ($)', 'R', 'F', 'M', 'RFM Score', 'Segment']
for c, h in enumerate(rfm_h, 1):
    ws5.cell(row=4, column=c, value=h)
style_header(ws5, 4, len(rfm_h))

# Write all customers (first 100 for readability, full RFM data)
rfm_display = rfm.head(100)
for r, (_, row) in enumerate(rfm_display.iterrows(), 5):
    ...
    ws5.cell(row=r, column=6, value=int(row['R_Score']))
    ws5.cell(row=r, column=7, value=int(row['F_Score']))
    ws5.cell(row=r, column=8, value=int(row['M_Score']))
    # RFM Score = R + F + M (formula)
    ws5.cell(row=r, column=9, value=f'=F{r}+G{r}+H{r}')
    ws5.cell(row=r, column=10, value=row['Segment'])
    ws5.cell(row=r, column=5).number_format = CURRENCY_FMT

rfm_end = 4 + len(rfm_display)
style_data_rows(ws5, 5, rfm_end, len(rfm_h))
set_col_widths(ws5, [14, 22, 14, 12, 14, 6, 6, 6, 10, 20])

# Segment summary with formulas referencing the table above
seg_start = rfm_end + 3
...
for r, seg in enumerate(seg_names, seg_start + 3):
    ws5.cell(row=r, column=1, value=seg)
    ws5.cell(row=r, column=2, value=f'=COUNTIF(J5:J{rfm_end},A{r})')
    total_row = seg_start + 3 + len(seg_names)
    ws5.cell(row=r, column=3, value=f'=IF(B{total_row}=0,0,B{r}/B{total_row})')
    ws5.cell(row=r, column=3).number_format = PCT_FMT
    ws5.cell(row=r, column=4, value=f'=SUMIF(J5:J{rfm_end},A{r},E5:E{rfm_end})')
    ws5.cell(row=r, column=4).number_format = CURRENCY_FMT
    ws5.cell(row=r, column=5, value=f'=IF(B{r}=0,0,D{r}/B{r})')
    ws5.cell(row=r, column=5).number_format = CURRENCY_FMT

seg_end = seg_start + 2 + len(seg_names)
tr = seg_end + 1
ws5.cell(row=tr, column=1, value='TOTAL').font = Font(name='Arial', bold=True, size=10)
ws5.cell(row=tr, column=2, value=f'=SUM(B{seg_start+3}:B{seg_end})')
ws5.cell(row=tr, column=2).font = Font(name='Arial', bold=True, size=10)
ws5.cell(row=tr, column=4, value=f'=SUM(D{seg_start+3}:D{seg_end})')
ws5.cell(row=tr, column=4).number_format = CURRENCY_FMT
ws5.cell(row=tr, column=4).font = Font(name='Arial', bold=True, size=10)
style_data_rows(ws5, seg_start + 3, tr, len(seg_h2))

# Pie chart — to the right of segment summary
pie2 = PieChart()
pie2.title = "Customer Segment Distribution"
pie2.style = 10
pie2.width = 14
pie2.height = 11
pie2.add_data(Reference(ws5, min_col=2, min_row=seg_start+3, max_row=seg_end))
pie2.set_categories(Reference(ws5, min_col=1, min_row=seg_start+3, max_row=seg_end))
pie2.dataLabels = DataLabelList()
pie2.dataLabels.showPercent = True
ws5.add_chart(pie2, f"G{seg_start}")
...
for r, (_, row) in enumerate(bottom10.iterrows(), 31):
    ...
    ws6.cell(row=r, column=3).number_format = CURRENCY_FMT
    ws6.cell(row=r, column=4).number_format = CURRENCY_FMT
    ws6.cell(row=r, column=4).font = Font(name='Arial', size=10, color='FF0000', bold=True)
    ws6.cell(row=r, column=5).number_format = PCT_FMT
...
# % of Revenue formulas
total_sales_cell = f'SUM(D46:D{abc_end})'
for r in range(46, abc_end + 1):
    ws6.cell(row=r, column=5, value=f'=IF({total_sales_cell}=0,0,D{r}/{total_sales_cell})')
    ws6.cell(row=r, column=5).number_format = PCT_FMT
...
tr = 8
ws7.cell(row=tr, column=1, value='TOTAL').font = Font(name='Arial', bold=True, size=10)
for c in [2, 3, 5]:
    cl = get_column_letter(c)
    ws7.cell(row=tr, column=c, value=f'=SUM({cl}4:{cl}7)')
    ws7.cell(row=tr, column=c).font = Font(name='Arial', bold=True, size=10)
...
# Bar chart — well below the table
bar_reg = BarChart()
bar_reg.type = "col"
...
ws7.add_chart(bar_reg, "A11")

# Top 15 States — starts after chart
state_start = 30
...
# Line chart — placed well below
line_ts = LineChart()
line_ts.title = "Revenue with Moving Averages"
...
ws8.add_chart(line_ts, f"A{ts_end + 3}")

# Seasonality table — after chart
season_start = ts_end + 22
...
for r, m in enumerate(range(1, 13), season_start + 3):
    ...
    ws8.cell(row=r, column=3).number_format = CURRENCY_FMT
    # Seasonality Index formula: this month's avg / overall average
    avg_range = f'C{season_start+3}:C{season_start+14}'
...
seas_end = season_start + 14
style_data_rows(ws8, season_start + 3, seas_end, len(seas_h))
...
for r, mode in enumerate(ship_modes, 4):
    ...
    total_orders = f'SUM(E4:E7)'
    ws9.cell(row=r, column=6, value=f'=IF({total_orders}=0,0,E{r}/{total_orders})')
...
tr = 8
ws9.cell(row=tr, column=1, value='TOTAL').font = Font(name='Arial', bold=True, size=10)
for c in [2, 3, 5]:
    cl = get_column_letter(c)
    ws9.cell(row=tr, column=c, value=f'=SUM({cl}4:{cl}7)')
    ws9.cell(row=tr, column=c).font = Font(name='Arial', bold=True, size=10)
...
for r, (label, count) in enumerate(del_dist.items(), del_start + 3):
    ws9.cell(row=r, column=1, value=str(label))
    ws9.cell(row=r, column=2, value=int(count))
    total_range = f'SUM(B{del_start+3}:B{del_start+7})'
    ws9.cell(row=r, column=3, value=f'=IF({total_range}=0,0,B{r}/{total_range})')
    ws9.cell(row=r, column=3).number_format = PCT_FMT

del_end = del_start + 7
style_data_rows(ws9, del_start + 3, del_end, len(del_h))
...
for r, (_, row) in enumerate(disc_analysis.iterrows(), 4):
    ...
    if row['Total Profit'] < 0:
        ws10.cell(row=r, column=5).font = Font(name='Arial', size=10, color='FF0000', bold=True)

disc_end = 3 + len(disc_analysis)
# Formula columns: % of Orders, Avg Profit/Order, Profit Margin
total_orders_range = f'SUM(B4:B{disc_end})'
for r in range(4, disc_end + 1):
    ws10.cell(row=r, column=3, value=f'=IF({total_orders_range}=0,0,B{r}/{total_orders_range})')
...
style_data_rows(ws10, 4, disc_end, len(disc_h))
set_col_widths(ws10, [16, 14, 12, 14, 14, 16, 14])

# Bar chart — below the table with plenty of space
bar_disc = BarChart()
bar_disc.type = "col"
...
bar_disc.width = 20
bar_disc.height = 12
bar_disc.add_data(Reference(ws10, min_col=7, min_row=3, max_row=disc_end), titles_from_data=True)
bar_disc.set_categories(Reference(ws10, min_col=1, min_row=4, max_row=disc_end))
ws10.add_chart(bar_disc, f"A{disc_end + 3}")

# Category x Discount analysis — after chart
cat_disc_start = disc_end + 22
...
# KPIs with formulas referencing Raw Data
kpi_labels = [
...
for r, cat in enumerate(categories, cat_kpi_row + 3):
    ...
    total_rev = f"SUM('Raw Data'!Q2:Q{LAST})"
    ws11.cell(row=r, column=6, value=f'=IF({total_rev}=0,0,B{r}/{total_rev})')
...
cat_kpi_end = cat_kpi_row + 5
style_data_rows(ws11, cat_kpi_row + 3, cat_kpi_end, len(cat_kpi_h))

# Regional summary with formulas
reg_kpi_row = cat_kpi_end + 3
add_title(ws11, 'Regional Performance', reg_kpi_row, 1)
for c, h in enumerate(['Region', 'Revenue', 'Profit', 'Margin', 'Orders', '% of Revenue'], 1):
...
reg_kpi_end = reg_kpi_row + 6
style_data_rows(ws11, reg_kpi_row + 3, reg_kpi_end, 6)
...
print(f'Raw Data rows: {N}')
print('All computed columns use Excel formulas (Profit Margin, Delivery Days, Moving Averages, etc.)')
